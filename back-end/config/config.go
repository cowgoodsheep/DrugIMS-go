...
// 服务器配置信息
type Server struct {
	IP   string `toml:"ip"`   // 服务器IP地址
	Port int    `toml:"port"` // 服务器端口号
}

// ai配置信息
type AiChat struct {
	URL   string `toml:"url"`   // 请求接口
	Key   string `toml:"key"`   // api key
	Model string `toml:"model"` // ai模型
}
